...
void Call::ReadFnName(std::istream& input)
{
  m_name = ReadWord(input);
}
...
Exp *Call::CreateStaySignal(const Pos& pos) const
{
  ...
  if(pos.needsSignal) {
    ...
    pStaySignal->SetSignalSlot(2 * pos.count + 2 * pos.needsSignalIndex + 1, -1);
  } else {
...
void Call::AppendTo(SimpleFn& fn, unsigned index, unsigned count, unsigned needsSignalIndex, unsigned needsSignalCount, Program& program, unsigned tempStart) const
{
  SimpleFn *pCall = GetFn(program);
  if(!pCall)
    return;

  Pos pos(pCall->NeedsSignal(), index, count, needsSignalIndex, needsSignalCount);
  ...
  for(unsigned i=0;i<pCall->Size();i++) {
    ...
    Exp *pTranslated = pExp->Translate(m_params, tempStart, 2 * count + 2 * needsSignalCount);
