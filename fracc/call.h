...
class Call {
 public:
  ...
  void AppendTo(SimpleFn& fn, unsigned index, unsigned count,
  ...
  struct Pos {
    Pos()
        : needsSignal(false),
          ...
          needsSignalIndex(0),
          needsSignalCount(0) {}
    Pos(bool needsSignal_, unsigned index_, unsigned count_,
        unsigned needsSignalIndex_, unsigned needsSignalCount_)
        : needsSignal(needsSignal_),
          index(index_),
          count(count_),
          needsSignalIndex(needsSignalIndex_),
          needsSignalCount(needsSignalCount_) {}
